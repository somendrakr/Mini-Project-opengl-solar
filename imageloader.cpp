...
Image* loadBMP(const char* filename) {
	...
	assert(!file.fail() || !"Could not find file");
	char buffer[2];
	file.read(buffer, 2);
	...
	case 40:
		...
		height = readInt(file);
		file.ignore(2);
		assert(readShort(file) == 24 || !"Image is not 24 bits per pixel");
		assert(readShort(file) == 0 || !"Image is compressed");
		break;
	case 12:
		//OS/2 V1
